...
from .send_mails import send_custom_mail, SUBJECTS, MESSAGES
        ...
        if {'first_name', 'middle_name', 'last_name', 'username', 'email', 'password', 'company', 'position'} \
            ...
            try:
                ...
                if user_serializer.is_valid():
                    ...
                    user.save()
                    send_custom_mail(user, SUBJECTS[0], MESSAGES[0])
...
class UserView(APIView):
    def get(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return response(False, 'authentication is required')
        user = request.user
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'password' in data:
            try:
                ...
                send_custom_mail(request.user, SUBJECTS[1], MESSAGES[1])
...
class ContactView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        return response(False, 'contacts not found')
...
class BasketView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if order_items:
            ...
            for order_item in order_items:
                shop = Shop.objects.filter(id=order_item['shop']).first()
                if shop.status is False:
                    order_items.remove(order_item)
                    return response(False, f'Shop {shop.name} does not accept new orders now. '
                                    f'Please remove items related to this shop and try again')
                else:
    ...
    def delete(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return response(False, 'authentication is required')
        delete_item_id = request.GET.get('id')
    ...
    def put(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return response(False, 'authentication is required')
        update_item_id = request.GET.get('id')
        data = request.data.get('update_data')
        if update_item_id and data:
            order = Order.objects.filter(user_id=request.user.id, status='basket').prefetch_related(
                'order_items').first()
            OrderItem.objects.filter(order_id=order.id, id=update_item_id).update(quantity=data['quantity'])
        ...
        if orders:
            ...
            for order in orders:
                ...
                for item in order.order_items.all():
                    if item.shop.id not in shops:
                        shops.append(item.shop.id)
                order = total_sum(order)
                order.total_sum = order.total_sum + (len(shops) * 200)  # delivery price depending on shops' count)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if {'order_id', 'contact_id'}.issubset(data):
            try:
                ...
                if order:
                    send_custom_mail(request.user, SUBJECTS[2], MESSAGES[2])
        ...
        if is_auth_shop(request):
            ...
            if update_file:
                ...
                if data:
                    ...
                    return response(False, 'insufficient arguments')
                return response(False, 'data load failed')
